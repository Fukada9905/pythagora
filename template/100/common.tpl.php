    ...
    <article>
        ...
        <section id="top_navigation" class="box_container">
            <ul>
                ...
                <li><a data-href="<?=URL?><?=$val["function_id"];?>/<?php echo ($val["params"]) ? "?".$val["params"]:"";?>" class="must_conditions hover_txt"><div class="img_wrap"><img src="<?=IMG_DIR?>/icon/<?=$val["icon_name"];?>" alt="<?=$val["menu_name"];?>"></div><?=$val["menu_name"];?></a></li>
                <?php endforeach;?>
            </ul>
        </section>
    </article>
...
<?php require_once(TEMPLATE_DIR . DS . "common_parts/contents_overray.tpl.php"); ?>
    ...
    <script src="<?=JS_DIR?>/100/jquery.index.js?ver=<?=date('ymdHis')?>"></script>
</body>
</html>
