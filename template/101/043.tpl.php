    ...
    <article>
				...
				<form id="status_input_area">
					<div class="input_area_wrapper">
						...
						<dl class="input_area_inner">
							<dt>センター</dt>
							<dd>
								<button type="button" class="btn btn_function" data-open-target="center"><i class="fa fa-arrow-circle-right" aria-hidden="true"></i>センター指定</button>
		...
		<section class="hidden_data">
			<form name="output_form">
				<input type="hidden" id="param_sntcd" name="sntcd" value="">
			...
			<div class="table_wrap">
				<table id="data_table" class="list_table">
					<thead>
						<tr>
							<th rowspan="2">業者名</th>
							<th rowspan="2"><?=$date_conditions_divide?></th>
							<?php $columns = 5;foreach ($data as $row) : ?>
							<th colspan="3"><?=$row["NNSINM"]?></th>
							<?php $columns += 3;endforeach;?>
							<th rowspan="2">合計/ケース</th>
							<th rowspan="2">合計/バラ</th>
							<th rowspan="2">合計/重量</th>
						</tr>
						<tr>
					...
					<tbody>
						<td colspan="<?=$columns?>" class="no_data">対象データがありません</td>
					</tbody>
				</table>
			</div>
		</section>
...
<?php require_once(TEMPLATE_DIR . DS . "common_parts/contents_overray.tpl.php"); ?>
    ...
    <script src="<?=JS_DIR?>/101/jquery.043.js?ver=<?=date('ymdHis')?>"></script>
</body>
</html>
