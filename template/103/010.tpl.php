    ...
    <article>
		...
		<section class="tab_area">
			<div class="box_container">
				...
				<div class="tab_controller" data-target-view="target-view-houmen">方面別補給物量</div>
			</div>
		</section>
					...
					<section class="box_container data_area" data-target-date="<?=$val?>">
						...
						<div class="table_wrap">
							<table id="shukkasouko_data_table_<?=str_replace("/","_",$val)?>" class="list_table">
								<thead>
								<tr>
									<th rowspan="2">センター名</th>
									<?php $columns = 3;foreach ($data as $row) : ?>
										<th colspan="2"><?=$row["NNSICD"]?></th>
										<?php $columns += 2;endforeach;?>
									<th colspan="2">合計</th>
							...
							<div class="table_wrap">
								<table id="houmen_data_table_<?=str_replace("/","_",$val)?>" class="list_table">
									...
									<tr>
										...
										<th rowspan="2">納品先名称</th>
										<?php $columns = 4;foreach ($data as $row) : ?>
											<th colspan="2"><?=$row["NNSICD"]?></th>
											<?php $columns += 2;endforeach;?>
										<th colspan="2">合計</th>
		...
		<section class="hidden_data">
			...
			<form name="output_form">
                ...
                <input type="hidden" name="process_divide" value="1">
			</form>
...
<?php require_once(TEMPLATE_DIR . DS . "common_parts/contents_overray.tpl.php"); ?>
    ...
    <script src="<?=JS_DIR?>/103/jquery.010.js?ver=<?=date('ymdHis')?>"></script>
</body>
</html>
