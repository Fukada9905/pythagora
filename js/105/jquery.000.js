...
$(document).ready(function(){
	...
	$(document).on("click",".btn_output_pdf",function(){
		...
		window.document.output_form_pdf.action = "/class/pdf/output.php?method=105000" ;
...
function getMainData(){
	...
	ajax.setParams('105000','get_main_data',params);
	$.when(
		ajax.getData(true)
	).done(function(data){
		setDisplay(data);
...
function DoSetDisplay(target,data){
	...
	var out = [];
	var exclusion = ['process_divide','user_id','NNSICD','JGSCD','SNTCD','UNSKSCD','work_id','prev_key','STATUS'];
