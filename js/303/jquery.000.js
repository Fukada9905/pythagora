...
$(document).ready(function(){
	...
	$(BTN_SEND).on("click",function(){
		var target_id = $(this).attr("data-target-id");
		jConfirm('この内容で登録してよろしいですか？', '送信確認',function(r) {
			if(r===true){
				DoUpdate(target_id);
			}
		});
	...
	$(".btn_return").on("click",function(){
		var target_id = $(this).attr("data-target-id");
		jConfirm('この内容で登録してよろしいですか？', '差戻確認',function(r) {
			if(r===true){
				DoCancel(target_id);
			}
		});
...
function getMainData(){
	...
	ajax.setParams('303000','get_main_data',params);
	$.when(
		ajax.getData(true)
	).done(function(data){
		setDisplay(data);
...
function DoSetDisplay(target,data){
	...
	data.forEach(function (val, index, arr) {
		...
		if(val.Status !== "2"){
			...
			out.push('<td class="ta_center"><input type="button" class="btn btn_detail" name="detail_btn_'+val.work_id+'" value="承認処理" data-target-id="'+val.work_id+'"></td>\\n');
		}
...
function getDetailData(target_id){
	...
	ajax.setParams('303000','get_detail_data',params);
	$.when(
		ajax.getData(true)
	).done(function(data){
...
function DoSetDetailDisplay(target,data){
	...
	var out = [];
	var exclusion = ['PYTStocktakingDate','JGSNM','SNTNM','NNSINM','Reporter','Reviewer','AuthorizerComment','IsModifyRow','Status','work_id','work_detail_id'];
	var head_content = ['PYTStocktakingDate','JGSNM','SNTNM','NNSINM','Reporter','Reviewer','AuthorizerComment'];
	...
	data.forEach(function (val, index, arr) {
		if(!draw_header){
			...
			$(".btn_return").attr("data-target-id",val.work_id);
		...
		for (var key in val) {
			if($.inArray(key, exclusion) === -1){
				if ( key.match(/KKTSR/)) {
					...
					out.push('<td>'+functions.nl2br(val[key])+'</td>\n');
				}
			}
		}
		out.push('</tr>\n');
...
function DoUpdate(target_id){
	...
	ajax.setParams('303000','update_data',params);
	$.when(
		ajax.getData(true)
	).done(function(data){
		$("#over_ray_tables").stop().fadeOut("fast");
...
function DoCancel(target_id){
	...
	var ajax = new Ajax();
	var params = {"work_id":target_id,"Comment":$("#Comment").val()};
	ajax.setParams('303000','cancel_data',params);
	$.when(
		ajax.getData(true)
	).done(function(data){
		$("#over_ray_tables").stop().fadeOut("fast");
