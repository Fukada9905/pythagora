...
$(document).ready(function(){
	...
	$(BTN_OUTPUT_CSV).click(function(){
		...
		window.document.output_form_csv.action = "/class/csv/output.php?method=202000" ;
	...
	$(document).on("click",".btn_print",function(){
		var window_name = "202000_pdf";
		var target_id = $(this).data("target-id");
		$("input[name='work_id']").val(target_id);
		
		window.open("", window_name) ;
		window.document.output_form_pdf.action = "/class/pdf/output.php?method=202000" ;
...
function getMainData(){
	...
	ajax.setParams('202000','get_main_data',params);
	$.when(
		ajax.getData(true)
	).done(function(data){
		setDisplay(data);
