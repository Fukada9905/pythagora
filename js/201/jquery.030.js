$(document).ready(function(){
	...
	$(BTN_OUTPUT_CSV).click(function(){
		window.document.output_form.action = "/class/csv/output.php?method=201030" ;
	...
	$(BTN_OUTPUT_PDF).click(function(){
		var window_name = "201030_pdf";
		$("#param_sntnm").val($("#selection_SNTCD_text").text());
		window.document.output_form_pdf.action = "/class/pdf/output.php?method=201030";
...
function getMainData(){
	...
	var ajax = new Ajax();
	ajax.setParams('201030','get_main_data',{});
	$.when(
		ajax.getData(true)
	).done(function(data){
		setDisplay(data);
