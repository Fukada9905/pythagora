$(document).ready(function(){
	...
	$(BTN_OUTPUT_CSV).click(function(){
		window.document.output_form.action = "/class/csv/output.php?method=201020" ;
	...
	$(BTN_OUTPUT_PDF).click(function(){
		var window_name = "201020_pdf";
		$("#param_sntnm").val($("#selection_SNTCD_text").text());
		window.document.output_form_pdf.action = "/class/pdf/output.php?method=201020";
...
function getMainData(){
	...
	var ajax = new Ajax();
	ajax.setParams('201020','get_main_data',{});
	$.when(
		ajax.getData(true)
	).done(function(data){
		setDisplay(data);
...
function setDisplay(data){
	...
	if(!data.length){
		...
		var out = [];
		var foot = [];
		var exclusion = ['DATA_DIVIDE','NNSICD','JGSCD_SK'];
		var tmp_dateymd = '';
