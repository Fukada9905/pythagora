$(document).ready(function(){
	...
	$(BTN_OUTPUT_CSV).click(function(){
		...
		window.document.output_form.action = "/class/csv/output.php?method=103010" ;
	...
	$(BTN_OUTPUT_PDF).click(function(){
		...
		var window_name = "103010_"+target_divide+"_"+target_date.replace(/\//g,'')+"pdf";
		
		window.document.output_form_pdf.action = "/class/pdf/output.php?method=103010&process_divide="+target_divide;
...
function getFirstData(){
	...
	ajax.setParams('103010','get_main_data',{"target_divide":1,"target_date":last_date});
	ajax2.setParams('103010','get_main_data',{"target_divide":2,"target_date":last_date});
...
function getMainData(target_divide,target_date){
	...
	ajax.setParams('103010','get_main_data',{"target_divide":target_divide,"target_date":target_date});
	$.when(
		ajax.getData(true)
	).done(function(data){
...
function setDisplay(target_divide,target_date,data){
	...
	if(target_divide === 1){
		...
		exclusion = ['NNSICD','NNSINM','JGSCD','NHNSKCD','DATA_DIVIDE'];
	}
	else{
		...
		target = $(target_wrap).find("#"+table_id + " tbody");
		exclusion = ['NNSICD','NNSINM','JGSCD','JGSNM','SNTCD','DATA_DIVIDE'];
	}
	...
	if(!data.length){
		...
		$(target_wrap).data("is-load",1);
		DataTableSetting(2,$(target_wrap),table_id);
	}
