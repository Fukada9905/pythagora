...
$(document).ready(function(){
	...
	$(BTN_OUTPUT_CSV).click(function(){
		...
		window.document.output_form_csv.action = "/class/csv/output.php?method=103020" ;
	...
	//印刷1
	$(".btn_output_pdf1").click(function(){
		var window_name = "103020_1_pdf";
		...
		window.document.output_form_pdf.action = "/class/pdf/output.php?method=103020&process_divide=1" ;
	...
	//印刷1
	$(".btn_output_pdf2").click(function(){
		var window_name = "103020_2_pdf";
		...
		window.open("", window_name) ;
		window.document.output_form_pdf.action = "/class/pdf/output.php?method=103020&process_divide=2" ;
		window.document.output_form_pdf.method = "POST" ;
		window.document.output_form_pdf.target = window_name ;
		window.document.output_form_pdf.submit() ;
	});
...
function getMainData(){
	...
	ajax.setParams('103020','get_main_data',params);
	$.when(
		ajax.getData(true)
	).done(function(data){
		setDisplay(data);
...
function DoSetDisplay(target,data){
	...
	var out = [];
	var exclusion = ['user_id','JGSCD','SNTCD','UNSKSCD','NNSICD','work_id','prev_key'];
