...
/////* Initialize */
	...
	this.footerRows = parseInt(options.footerRows || "1");
...
/////* Create the framework dom */
	...
	this.sFFooter = this.sBase.cloneNode(false);
	this.sFooter = this.sBase.cloneNode(false);
	this.sFooterInner = this.sBase.cloneNode(false);
...
/////* Attach the required classNames */
	...
	this.sFFooter.className = "sFFooter";
	this.sFooter.className = "sFooter";
	this.sFooterInner.className = "sFooterInner";
...
/////* Clone parts of the data table for the new header table */
	...
	this.sFooterTable = this.sDataTable.cloneNode(false);
	if (this.sDataTable.tFoot) {
		this.sFooterTable.appendChild(this.sDataTable.tFoot.cloneNode(true));
	}
	else {
		//this.sFooterTable.appendChild(this.sDataTable.tBodies[0].cloneNode(false));
	}
	
	this.sFooterInner.appendChild(this.sFooterTable);
	...
	if (this.fixedCols > 0) {
		...
		this.sFFooterTable = this.sFooterTable.cloneNode(true);
		this.sFFooter.appendChild(this.sFFooterTable);
	}
...
/////* Set up the colGroup */
	...
	for (i=0, j=this.sDataTable.tBodies[0].rows[cleanRow].cells.length; i<j; i++) {
		if (i === this.colWidths.length || this.colWidths[i] === -1) {
			this.colWidths[i] = this.sDataTable.tBodies[0].rows[cleanRow].cells[i].offsetWidth;
		}
	}
	...
	this.sFooterTable.insertBefore(this.sColGroup.cloneNode(true), this.sFooterTable.firstChild);
	if (this.fixedCols > 0) {
...
/////* Style the tables individually if applicable */
	if (this.cssSkin !== "") {
		...
		this.sFooterTable.className += " " + this.cssSkin + "-Footers";
		if (this.fixedCols > 0) {
			this.sFDataTable.className += " " + this.cssSkin + "-Fixed";
			this.sFHeaderTable.className += " " + this.cssSkin + "-FixedHeaders";
			this.sFFooterTable.className += " " + this.cssSkin + "-FixedFooters";
		}
	}
...
/////* Throw everything into sBase */
	...
	this.sFooter.appendChild(this.sFooterInner);
	...
	this.sBase.appendChild(this.sFooter);
...
/////* Align the tables */
	...
	this.sFooterHeight = this.sFooterTable.offsetHeight;
	
	
	sDataTableStyles = "margin-top: " + (this.sHeaderHeight * -1) + "px;";
	sDataTableStyles += "margin-bottom: " + (this.sFooterHeight * -1) + "px;";
	sDataStyles = "margin-top: " + this.sHeaderHeight + "px;";
	sDataStyles += "margin-bottom: " + this.sFooterHeight + "px;";
	sDataStyles += "height: " + (this.sParentHeight - this.sHeaderHeight - this.sFooterHeight) + "px;";
	...
	if((this.sDataTable.tFoot)){
		this.sDataTable.tFoot.style.cssText = "display:none;";
	}
	...
	(function (st) {
		if (st.fixedCols > 0) {
			st.sData.onscroll = function () {
				...
				st.sFooterInner.style.right = st.sData.scrollLeft + "px";
