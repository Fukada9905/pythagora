$(document).ready(function(){
	...
	$(BTN_OUTPUT_CSV).click(function(){
		window.document.output_form.action = "/class/csv/output.php?method=101042" ;
		window.document.output_form.method = "POST" ;
		window.document.output_form.submit() ;
	});
	//PDF出力ボタン
	$(BTN_OUTPUT_PDF).click(function(){
		var window_name = "101042_pdf";
		
		window.document.output_form_pdf.action = "/class/pdf/output.php?method=101042" ;
...
function getMainData(){
	...
	var ajax = new Ajax();
	ajax.setParams('101042','get_main_data',{});
	$.when(
		ajax.getData(true)
	).done(function(data){
		setDisplay(data);
