...
$(document).ready(function(){
	...
	$(BTN_OUTPUT_CSV).click(function(){
		...
		window.document.output_form.action = "/class/csv/output.php?method=101013" ;
	...
	$(BTN_OUTPUT_PDF).click(function(){
		var window_name = "101013_pdf";
		$("#param_sntnm").val($("#selection_SNTCD_text").text());
		window.document.output_form_pdf.action = "/class/pdf/output.php?method=101013";
...
function getMainData(data){
	...
	ajax.setParams('101013','get_main_data',data);
	$.when(
		ajax.getData(true)
	).done(function(data){
		setDisplay(data);
