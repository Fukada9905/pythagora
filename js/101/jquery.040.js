...
$(document).ready(function(){
	...
	$(".btn_output_page2").click(function(){
		...
		SetStorage(params);
		window.location.href = "/101042/";
	});

	//画面3
	$(".btn_output_page3").click(function(){
		...
		SetStorage(params);
		window.location.href = "/101043/";
	});

	//画面4
	$(".btn_output_page4").click(function(){
		...
		SetStorage(params);
		window.location.href = "/101044/";
	});

	//画面5
	$(".btn_output_page5").click(function(){
		...
		SetStorage(params);
		window.location.href = "/101045/";
	});

	var data = JSON.parse(sessionStorage.getItem('101040_DATA'));
	if(data !== null){
...
function SetStorage(selection){
	sessionStorage.setItem('101040_DATA',JSON.stringify(_data));

	var params = {
			'101040_SELECTION':selection
	};
...
function getMainData(){
	...
	ajax.setParams('101040','get_main_data',params);
	$.when(
		ajax.getData(true)
	).done(function(data){
		setDisplay(data);
	...
	var params2 = {
			'101040_DATE_DIVIDE':getVal('input[name=date_divide]')
		,	'101040_DATE_FROM':getVal('input[name=date_from]')
		,	'101040_DATE_TO':getVal('input[name=date_to]')
		,	'101040_NINUSHI':JSON.parse(sessionStorage.getItem(LS_NINUSHI))
	};
