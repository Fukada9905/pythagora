$(document).ready(function(){
	...
	$(BTN_OUTPUT_CSV).click(function(){
		window.document.output_form.action = "/class/csv/output.php?method=101041" ;
	...
	$(BTN_OUTPUT_PDF).click(function(){
		var window_name = "101041_pdf";
		
		window.document.output_form_pdf.action = "/class/pdf/output.php?method=101041" ;
...
function getMainData(){
	...
	var ajax = new Ajax();
	ajax.setParams('101041','get_main_data',{});
	$.when(
		ajax.getData(true)
	).done(function(data){
		setDisplay(data);
...
function setDisplay(data){
	...
	if(!data.length){
		...
		var out = [];
		var foot = [];
		var exclusion = ['DATA_DIVIDE','NNSICD','NNSINM','JGSCD','JGSNM','SNTCD'];
