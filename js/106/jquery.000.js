...
$(document).ready(function(){
	...
	$(document).on("click",".btn_output_csv",function(){
		...
		window.document.output_form_csv.action = "/class/csv/output.php?method=106000" ;
...
function getMainData(){
	...
	ajax.setParams('106000','get_main_data',params);
	$.when(
		ajax.getData(true)
	).done(function(data){
		setDisplay(data);
...
function DoSetDisplay(target,data){
	...
	var out = [];
	var exclusion = ['process_divide','user_id','shipper_code','sales_office_code','work_id','prev_key','STATUS'];
