DROP PROCEDURE IF EXISTS usp_205000_details;
delimiter //

CREATE PROCEDURE usp_205000_details(
	IN p_work_id int
...
BEGIN
    ...
    ,	ufn_get_datetime_format(WK.SYORIYMD) AS SYORIYMD
    ...
    ,	CONCAT(WD.Registrant,' (',ufn_get_date_format(WD.Registdatetime),')') AS Registrant
    ,	CASE WHEN WD.KKTSR1 != WD.JitsuCase || WD.KKTSR3 != WD.JitsuBara THEN 1 ELSE 0 END AS IsModifyRow
    ,	WK.Status
    ,	WK.work_id
	FROM wk_205000_details AS WD
    INNER JOIN wk_205000_head AS WK
