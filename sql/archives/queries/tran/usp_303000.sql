DROP PROCEDURE IF EXISTS usp_303000;
delimiter //

CREATE PROCEDURE usp_303000(
...
BEGIN
    ...
    BEGIN
		DELETE FROM wk_303000_head WHERE user_id = p_user_id;
    ...
    IF EXISTS(SELECT * FROM tmp_details) THEN
		...
		SELECT IFNULL(MAX(work_id),0) + 1 INTO @def_work_id FROM wk_303000_head FOR UPDATE;
		SELECT IFNULL(MAX(work_detail_id),0) + 1 INTO @work_detail_id FROM wk_303000_details FOR UPDATE;
		...
		SET @sql_head = 'INSERT INTO wk_303000_head SELECT';
		...
		SET @sql_ins = 'INSERT INTO wk_303000_details SELECT';
        ...
        ,	CASE WHEN WK.Status = 2 THEN '承認' ELSE '' END AS IsConfirm
        ,	WK.Status
        FROM wk_303000_head AS WK
        INNER JOIN wk_303000_details AS WD
			ON WK.work_id = WD.work_id
        WHERE WK.user_id = p_user_id
        AND WK.Status NOT IN(3)
        GROUP BY
        	WK.work_id
    ...
    ELSE
		-- NOTHINGS TO RETURN
		SELECT * FROM wk_303000_head WHERE user_id = p_user_id;
    END IF;          
