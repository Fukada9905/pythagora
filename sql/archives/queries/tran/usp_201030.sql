DROP PROCEDURE IF EXISTS usp_201030;
delimiter //

CREATE PROCEDURE usp_201030(
...
BEGIN
	...
	DELETE FROM wk_201030 WHERE user_id = p_user_id;
    ...
    IF EXISTS(SELECT * FROM tmp_data LIMIT 1) THEN
		...
		SELECT IFNULL(MAX(work_id),0) INTO @work_id FROM wk_201030 FOR UPDATE;
        
        INSERT INTO wk_201030
        SELECT
			@work_id:=@work_id+1
		,	p_user_id
		,	DT.NOHINYMD
