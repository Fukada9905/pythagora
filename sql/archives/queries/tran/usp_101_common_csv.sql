DROP PROCEDURE IF EXISTS usp_101_common_csv;
delimiter //

CREATE PROCEDURE usp_101_common_csv(
...
BEGIN
    ...
    IF p_process_divide = 1 THEN
		SET @sql_sel = CONCAT(@sql_sel, '  SH.DKBNNM AS \'データ区分名称\'');		
	ELSE
    ...
    IF p_process_divide = 1 THEN
		SET @sql_sel = CONCAT(@sql_sel, ' , SD.SHNKKKMEI AS \'規格\'');	
	ELSE
    ...
    IF p_process_divide = 1 THEN
		SET @sql_sel = CONCAT(@sql_sel, ' , SD.JCSR1 AS \'ケース数量\'');
		SET @sql_sel = CONCAT(@sql_sel, ' , (SD.JCSR1 DIV SD.PL) AS \'パレット枚数\'');
		SET @sql_sel = CONCAT(@sql_sel, ' , (SD.JCSR1 MOD SD.PL) AS \'パレット端数\'');
		SET @sql_sel = CONCAT(@sql_sel, ' , SD.JCSR2 AS \'中間数量\'');
		SET @sql_sel = CONCAT(@sql_sel, ' , SD.JCSR3 AS \'バラ数量\'');
		SET @sql_sel = CONCAT(@sql_sel, ' , SD.JCSSR AS \'総バラ数量\'');
		SET @sql_sel = CONCAT(@sql_sel, ' , TRUNCATE(SD.JCSJR+0.9, 0) AS \'総重量\'');
    ELSE
    ...
    IF p_process_divide = 1 THEN
		SET @sql_sel = CONCAT(@sql_sel, '  FROM shipment_provisionals AS SH');
        SET @sql_sel = CONCAT(@sql_sel, '  INNER JOIN shipment_provisional_details AS SD');
    ...
    IF p_sntcd IS NOT NULL THEN
		...
		SET @sql_sel = CONCAT(@sql_sel,' ON SD.JGSCD = ST.JGSCD');
		SET @sql_sel = CONCAT(@sql_sel,' AND SD.SNTCD = ST.SNTCD');
	END IF;
	...
	SET @sql_sel = CONCAT(@sql_sel,@sql_where,' ORDER BY SH.NNSICD,SD.JGSCD,SD.SNTCD,SH.SYUKAYMD, SH.DENNO, SD.DENGNO');
