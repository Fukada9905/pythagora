DROP PROCEDURE IF EXISTS usp_304000;
delimiter //

CREATE PROCEDURE usp_304000(
...
BEGIN
    ...
    BEGIN
		DELETE FROM wk_304000_head WHERE user_id = p_user_id;
    ...
    IF EXISTS(SELECT * FROM tmp_details) THEN
		...
		SELECT IFNULL(MAX(work_id),0) + 1 INTO @def_work_id FROM wk_304000_head FOR UPDATE;
		SELECT IFNULL(MAX(work_detail_id),0) + 1 INTO @work_detail_id FROM wk_304000_details FOR UPDATE;
		...
		SET @sql_head = 'INSERT INTO wk_304000_head SELECT';
		...
		SET @sql_ins = 'INSERT INTO wk_304000_details SELECT';
        ...
        ,	WK.Status
        FROM wk_304000_head AS WK
        INNER JOIN wk_304000_details AS WD
    ...
    ELSE
		-- NOTHINGS TO RETURN
		SELECT * FROM wk_304000_head WHERE user_id = p_user_id;
    END IF;          
