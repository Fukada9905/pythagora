DROP PROCEDURE IF EXISTS usp_204000;
delimiter //

CREATE PROCEDURE usp_204000(
...
BEGIN
    ...
    BEGIN
    	ROLLBACK;
		DELETE FROM wk_204000_head WHERE user_id = p_user_id;
    ...
    SET @sql_ins = CONCAT(@sql_ins, '  AD.ReportDatetime DESC');		    
    ...
    SET @sql_ins = CONCAT(@sql_ins, ', AH.NOHINYMD ASC');		
    ...
    IF EXISTS(SELECT * FROM tmp_details) THEN
		...
		SELECT IFNULL(MAX(work_id),0) + 1 INTO @def_work_id FROM wk_204000_head FOR UPDATE;
		SELECT IFNULL(MAX(work_detail_id),0) + 1 INTO @work_detail_id FROM wk_204000_details FOR UPDATE;
		...
		SET @sql_head = 'INSERT INTO wk_204000_head SELECT';
		...
		SET @sql_ins = 'INSERT INTO wk_204000_details SELECT';
		...
		SELECT
			WK.work_id
		,	ufn_get_datetime_format(WK.ReportDatetime) AS ReportDatetime		
        ,	WK.NNSICD
        ,	WK.NNSINM
        ,	WK.JGSCD_SK
        ,	WK.JGSNM_SK
        ...
        ,	WK.Status
        FROM wk_204000_head AS WK
        INNER JOIN wk_204000_details AS WD
    ...
    ELSE
		-- NOTHINGS TO RETURN
		SELECT * FROM wk_204000_head WHERE user_id = p_user_id;
    END IF;          
