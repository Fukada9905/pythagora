...
,	PRIMARY KEY (JGSCD,SNTCD)
,	KEY IX_mtb_center_JGSCD (JGSCD)
,	KEY IX_mtb_center_del_flag (del_flag)
,	CONSTRAINT FK_mtb_center_mtb_jigyosho
		FOREIGN KEY (JGSCD) REFERENCES mtb_jigyosho (JGSCD)
		ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='センターマスタ (事業所に紐づくセンターの管理を行う)';//

/** INSERT(BEFORE) トリガー **/
CREATE TRIGGER mtb_center_BEFORE_INSERT BEFORE INSERT ON mtb_center FOR EACH ROW
BEGIN
...
/** INSERT トリガー **/
CREATE TRIGGER mtb_center_AFTER_INSERT AFTER INSERT ON mtb_center FOR EACH ROW
BEGIN
	INSERT INTO mtb_center_history(
...
/** UPDATE(BEFORE) トリガー **/
CREATE TRIGGER mtb_center_BEFORE_UPDATE BEFORE UPDATE ON mtb_center FOR EACH ROW
BEGIN
...
/** UPDATE トリガー **/
CREATE TRIGGER mtb_center_AFTER_UPDATE AFTER UPDATE ON mtb_center FOR EACH ROW
BEGIN
	INSERT INTO mtb_center_history(
			history_record_divide
		,	JGSCD
...
/** DELETE トリガー **/
CREATE TRIGGER mtb_center_AFTER_DELETE AFTER DELETE ON mtb_center FOR EACH ROW
BEGIN
	INSERT INTO mtb_center_history(
			history_record_divide
		,	JGSCD
