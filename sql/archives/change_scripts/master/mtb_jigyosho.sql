delimiter //
/** テーブル作成 **/
CREATE TABLE mtb_jigyosho(
	JGSCD						varchar(10)			NOT NULL							COMMENT '事業所コード'
,	JGSNM						nvarchar(50)		DEFAULT NULL						COMMENT '事業所名称'
...
,	PRIMARY KEY (JGSCD)
,	KEY IX_mtb_jigyosho_del_flag (del_flag)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='事業所マスタ (事業所の管理を行う)';//

/** INSERT(BEFORE) トリガー **/
CREATE TRIGGER mtb_jigyosho_BEFORE_INSERT BEFORE INSERT ON mtb_jigyosho FOR EACH ROW
BEGIN
...
/** INSERT トリガー **/
CREATE TRIGGER mtb_jigyosho_AFTER_INSERT AFTER INSERT ON mtb_jigyosho FOR EACH ROW
BEGIN
	INSERT INTO mtb_jigyosho_history(
...
/** UPDATE(BEFORE) トリガー **/
CREATE TRIGGER mtb_jigyosho_BEFORE_UPDATE BEFORE UPDATE ON mtb_jigyosho FOR EACH ROW
BEGIN
...
/** UPDATE トリガー **/
CREATE TRIGGER mtb_jigyosho_AFTER_UPDATE AFTER UPDATE ON mtb_jigyosho FOR EACH ROW
BEGIN
	INSERT INTO mtb_jigyosho_history(
			history_record_divide
		,	JGSCD
...
/** DELETE トリガー **/
CREATE TRIGGER mtb_jigyosho_AFTER_DELETE AFTER DELETE ON mtb_jigyosho FOR EACH ROW
BEGIN
	INSERT INTO mtb_jigyosho_history(
			history_record_divide
		,	JGSCD
