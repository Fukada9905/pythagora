DROP PROCEDURE IF EXISTS pyt_p_104000;
DELIMITER //
CREATE PROCEDURE pyt_p_104000(
...
BEGIN
    ...
    BEGIN
		ROLLBACK;
		DELETE FROM pyt_w_104000_head WHERE user_id = p_user_id AND process_divide = p_process_divide;
    ...
    -- SET @sql_ins = CONCAT(@sql_ins, ', SH.fraction AS KKTSR3');
    ...
    SET @sql_ins = CONCAT(@sql_ins, ', SH.shipping_location_code ASC');
    ...
    IF EXISTS(SELECT * FROM tmp_details) THEN
        ...
        SET @sql_prev_key = CONCAT(@sql_prev_key, ' , IFNULL(SYUKAP,\'\')');
		...
		SET @sql_head = 'INSERT INTO pyt_w_104000_head SELECT DISTINCT';
        ...
        SET @sql_head = CONCAT(@sql_head, ', SYUKAP');
		SET @sql_head = CONCAT(@sql_head, ', UNSKSCD AS UNSKSCD');
		SET @sql_head = CONCAT(@sql_head, ', UNSKSNM AS UNSKSNM');        
		...
		SET @prevkey:=null;
		SET @work_id = 0;
        SET @work_detail_id = 0;
			
		SET @sql_ins = 'INSERT INTO pyt_w_104000_details SELECT';
        ...
        -- STEP.5 RETURN
		SELECT * FROM pyt_w_104000_head WHERE user_id = p_user_id AND process_divide = p_process_divide
        ORDER BY SYORIYMD DESC, NOHINYMD DESC, NNSICD ASC, JGSCD ASC, SYUKAP ASC, UNSKSCD ASC;
    ELSE
		-- NOTHINGS TO RETURN
		SELECT * FROM pyt_w_104000_head WHERE user_id = p_user_id AND process_divide = p_process_divide;
    END IF;          
