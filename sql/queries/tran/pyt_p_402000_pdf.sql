DROP PROCEDURE IF EXISTS pyt_p_402000_pdf;
delimiter //

CREATE PROCEDURE pyt_p_402000_pdf(
	IN p_target_id text
...
BEGIN
	...
	,	CONCAT(IFNULL(WK.delivery_code,''),'\r\n',IFNULL(WK.delivery_name,''),'　',IFNULL(WK.delivery_address,'')) AS delivery
    ,	CONCAT(IFNULL(WK.transporter_code,''),'\r\n',IFNULL(WK.transporter_name,'')) AS transporter
	,	WK.work_id
    ,	WK.target_divide
    ,	SD.id
    ,	SD.detail_no
    ...
    SELECT * FROM tmp_data ORDER BY warehouse_accounting_date, work_id, detail_no;
