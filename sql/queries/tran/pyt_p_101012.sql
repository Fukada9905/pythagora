DROP PROCEDURE IF EXISTS pyt_p_101012;
delimiter //


CREATE PROCEDURE pyt_p_101012(
...
BEGIN
    ...
    BEGIN
		...
		DROP TABLE IF EXISTS tmp_data_jigyousho_total;
    ...
    IF EXISTS(SELECT * FROM tmp_data LIMIT 1) THEN
        ...
        SET @sql_ret = 'CREATE TEMPORARY TABLE tmp_data_jigyousho_total';
		SET @sql_ret = CONCAT(@sql_ret,' SELECT');
		SET @sql_ret = CONCAT(@sql_ret,'  PTNCD');
		SET @sql_ret = CONCAT(@sql_ret,' ,PTNNM');
        SET @sql_ret = CONCAT(@sql_ret,' ,JGSCD');
		SET @sql_ret = CONCAT(@sql_ret,' ,CONCAT(IFNULL(JGSNM,\'\'),\' 集計\') AS JGSNM');			
		...
		SET @sql_ret = CONCAT(@sql_ret,' ,SUM(TOTAL_KKTSR3) AS TOTAL_KKTSR3');
		SET @sql_ret = CONCAT(@sql_ret,' ,SUM(TOTAL_WGT) AS TOTAL_WGT');
        SET @sql_ret = CONCAT(@sql_ret,' ,4 AS DATA_DIVIDE');
        SET @sql_ret = CONCAT(@sql_ret,' FROM tmp_data AS DD');
        
        PREPARE _stmt_ret4 from @sql_ret;
		EXECUTE _stmt_ret4;
		DEALLOCATE PREPARE _stmt_ret4;
        
        SELECT * FROM tmp_data_details
		UNION ALL
		SELECT * FROM tmp_data_jigyousho_total
		UNION ALL
        SELECT * FROM tmp_data_ptncd_total
        UNION ALL
		SELECT * FROM tmp_data_total
		ORDER BY PTNCD,JGSCD,DATA_DIVIDE,DATEYMD;
