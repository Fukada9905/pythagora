DROP PROCEDURE IF EXISTS pyt_p_304000_pdf;
delimiter //

CREATE PROCEDURE pyt_p_304000_pdf(
	IN p_work_id int
...
BEGIN
    ...
    ,	pyt_ufn_get_date_format(WD.ReportDatetime) AS '報告日'
    ,	CONCAT('大塚倉庫 ',WH.JGSNM) AS '確認者会社名'
    ,	WD.ReviewerName AS '確認者名前'
    ,	pyt_ufn_get_date_format(WD.ReviewDatetime) AS '確認日'
    ,	CONCAT('大塚倉庫 ',WH.JGSNM) AS '承認者会社名'
    ,	WD.AuthorizerName AS '承認者名前'
    ,	pyt_ufn_get_date_format(WD.AuthorizerDatetime) AS '承認日'
	FROM pyt_w_304000_details AS WD
    INNER JOIN pyt_w_304000_head AS WH
