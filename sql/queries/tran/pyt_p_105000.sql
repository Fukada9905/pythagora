DROP PROCEDURE IF EXISTS pyt_p_105000;
DELIMITER //
CREATE PROCEDURE pyt_p_105000(
...
BEGIN
    ...
    BEGIN
		ROLLBACK;
		DELETE FROM pyt_w_105000_head WHERE user_id = p_user_id AND process_divide = p_process_divide;
    ...
    IF EXISTS(SELECT * FROM tmp_details) THEN
		...
		SET @sql_head = 'INSERT INTO pyt_w_105000_head SELECT DISTINCT';
		...
		SET @sql_ins = 'INSERT INTO pyt_w_105000_details SELECT';
        ...
        -- STEP.5 RETURN
		SELECT * FROM pyt_w_105000_head WHERE user_id = p_user_id AND process_divide = p_process_divide
        ORDER BY SYORIYMD DESC, NOHINYMD DESC, NNSICD ASC, JGSCD ASC, SNTCD ASC, SYUKAP ASC, UNSKSCD ASC;
    ELSE
		-- NOTHINGS TO RETURN
		SELECT * FROM pyt_w_105000_head WHERE user_id = p_user_id AND process_divide = p_process_divide;
    END IF;          
