DROP PROCEDURE IF EXISTS pyt_p_103040;
delimiter //

CREATE PROCEDURE pyt_p_103040(
...
BEGIN
    ...
    BEGIN
		ROLLBACK;
		DELETE FROM pyt_w_103040_head WHERE user_id = p_user_id AND process_divide = p_process_divide;
    ...
    SET @sql_ins = CONCAT(@sql_ins,', IFNULL(PN.PTNCD,\'ZZZZZZZZZ\') AS PTNCD');
    ...
    SET @sql_ins = CONCAT(@sql_ins, ', IFNULL(PN.PTNCD,\'ZZZZZZZZZ\') ASC');
    ...
    IF EXISTS(SELECT * FROM tmp_details LIMIT 1) THEN    
        ...
        SET @sql_prev_key = CONCAT(@sql_prev_key, ' , IFNULL(PTNCD,\'ZZZZZZZZZ\')');
		...
		SET @sql_head = 'INSERT INTO pyt_w_103040_head SELECT DISTINCT';
        ...
        SET @sql_head = CONCAT(@sql_head, ', PTNCD');
		...
		SET @sql_ins = 'INSERT INTO pyt_w_103040_details SELECT';
		...
		SELECT
			*,
            1 AS checked
        FROM pyt_w_103040_head 
        WHERE user_id = p_user_id AND process_divide = p_process_divide
        ORDER BY SYORIYMD DESC, DATEYMD DESC, JGSCD ASC, PTNCD ASC, NNSICD ASC;
        
    ELSE
		SELECT * FROM pyt_w_103040_head WHERE user_id = p_user_id AND process_divide = p_process_divide;
    END IF;
