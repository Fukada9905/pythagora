DROP PROCEDURE IF EXISTS pyt_p_101030;
DELIMITER //
CREATE PROCEDURE pyt_p_101030(
...
BEGIN
    ...
    BEGIN
		ROLLBACK;
		DELETE FROM pyt_w_101030_head WHERE user_id = p_user_id AND process_divide = p_process_divide;
	...
	IF p_process_divide = 1 THEN
		SET @sql_ins = CONCAT(@sql_ins, '  CASE WHEN SH.order_data_type IN(\'126\',\'252\',\'253\',\'254\',\'255\',\'546\') THEN \'仮出荷AM\' ELSE \'仮出荷PM\' END AS DKBNNM');		
	ELSE
    ...
    IF p_process_divide = 1 THEN
		SET @sql_ins = CONCAT(@sql_ins, ' , null AS JCDENNO');
    ELSE
    ...
    */
    ...
    SET @sql_from = '';
    IF p_process_divide = 1 THEN
		SET @sql_from = CONCAT(@sql_from, ' FROM t_provisional_shipment AS SH');                
    ELSE
    ...
    END IF;
    SET @sql_ins = CONCAT(@sql_ins, ', SH.sales_office_code DESC');
    IF p_pt_name_divide = 1 THEN
		SET @sql_ins = CONCAT(@sql_ins, ', IFNULL(PN.PTNCD,\'ZZZZZZZZZZ\') ASC');
    ELSE
		SET @sql_ins = CONCAT(@sql_ins, ', IFNULL(SH.transporter_code,\'ZZZZZZZZZZ\') ASC');
    END IF;
    ...
    IF EXISTS(SELECT * FROM tmp_details) THEN
		...
		SELECT IFNULL(MAX(work_id),0) + 1 INTO @def_work_id FROM pyt_w_101030_head FOR UPDATE;
		SELECT IFNULL(MAX(work_detail_id),0) + 1 INTO @work_detail_id FROM pyt_w_101030_details FOR UPDATE;
		...
		SET @sql_head = 'INSERT INTO pyt_w_101030_head SELECT DISTINCT';
		...
		SET @sql_ins = 'INSERT INTO pyt_w_101030_details SELECT';
        ...
        -- STEP.5 RETURN
		SELECT * FROM pyt_w_101030_head WHERE user_id = p_user_id AND process_divide = p_process_divide
        ORDER BY SYORIYMD DESC, DATEYMD DESC, JGSCD ASC, IFNULL(PTNCD,'ZZZZZZZZZZ') ASC, NNSICD ASC;
    ELSE
		-- NOTHINGS TO RETURN
		SELECT * FROM pyt_w_101030_head WHERE user_id = p_user_id AND process_divide = p_process_divide;
    END IF;          
