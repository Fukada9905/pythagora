DROP PROCEDURE IF EXISTS pyt_p_101040;
delimiter //

CREATE PROCEDURE pyt_p_101040(
...
BEGIN
    ...
    BEGIN
		ROLLBACK;
		DELETE FROM pyt_w_101040_head WHERE user_id = p_user_id AND process_divide = p_process_divide;
    ...
    IF EXISTS(SELECT * FROM tmp_details LIMIT 1) THEN    
		...
		SET @sql_head = 'INSERT INTO pyt_w_101040_head SELECT DISTINCT';
		...
		SET @sql_ins = 'INSERT INTO pyt_w_101040_details SELECT';
    ...
    ELSE
		SELECT * FROM pyt_w_101040_head WHERE user_id = p_user_id AND process_divide = p_process_divide;
    END IF;
