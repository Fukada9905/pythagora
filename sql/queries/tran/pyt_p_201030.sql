DROP PROCEDURE IF EXISTS pyt_p_201030;
delimiter //

CREATE PROCEDURE pyt_p_201030(
...
BEGIN
	...
	DELETE FROM pyt_w_201030 WHERE user_id = p_user_id;
    ...
    IF EXISTS(SELECT * FROM tmp_data LIMIT 1) THEN
		...
		SELECT IFNULL(MAX(work_id),0) INTO @work_id FROM pyt_w_201030 FOR UPDATE;
        
        INSERT INTO pyt_w_201030
        SELECT
			@work_id:=@work_id+1
		,	p_user_id
		,	DT.NOHINYMD
        ...
        (
			...
			SELECT
				*
			FROM tmp_data_total
			ORDER BY NOHINYMD,DATA_DIVIDE,NNSICD,SHCD,RTNO
        ) AS DT;
