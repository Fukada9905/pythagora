DROP PROCEDURE IF EXISTS pyt_p_203000;
delimiter //

CREATE PROCEDURE pyt_p_203000(
...
BEGIN
    ...
    BEGIN
    	ROLLBACK;
		DELETE FROM pyt_w_203000_head WHERE user_id = p_user_id;
	...
	SET @sql_ins = CONCAT(@sql_ins, ' , pyt_ufn_get_KKTSR(AH.package_count, AH.total_fraction, AH.quantity_per_package) AS JitsuCase');
    SET @sql_ins = CONCAT(@sql_ins, ' , CASE WHEN (AH.fraction <> 0) THEN AH.total_fraction ELSE 0 END AS JitsuBara');
    ...
    IF EXISTS(SELECT * FROM tmp_details) THEN
		...
		SELECT IFNULL(MAX(work_id),0) + 1 INTO @def_work_id FROM pyt_w_203000_head FOR UPDATE;
		SELECT IFNULL(MAX(work_detail_id),0) + 1 INTO @work_detail_id FROM pyt_w_203000_details FOR UPDATE;
		...
		SET @sql_head = 'INSERT INTO pyt_w_203000_head SELECT';
		...
		SET @sql_ins = 'INSERT INTO pyt_w_203000_details SELECT DISTINCT';
        ...
        -- STEP.5 RETURN
		SELECT * FROM pyt_w_203000_head WHERE user_id = p_user_id
