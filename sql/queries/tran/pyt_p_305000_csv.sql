DROP PROCEDURE IF EXISTS pyt_p_305000_csv;
delimiter //

CREATE PROCEDURE pyt_p_305000_csv(
	IN p_ids text
...
BEGIN
    ...
    SET @sql_sel = 'SELECT';
	SET @sql_sel = CONCAT(@sql_sel, '   NULL AS \'在庫管理ID\'');
    ...
    SET @sql_sel = CONCAT(@sql_sel,' FROM pyt_w_305000_details AS WD');
    SET @sql_sel = CONCAT(@sql_sel,' INNER JOIN pyt_w_305000_head AS WK');
    SET @sql_sel = CONCAT(@sql_sel,' ON WD.work_id = WK.work_id');
    SET @sql_sel = CONCAT(@sql_sel,' AND WD.user_id = WK.user_id');  
