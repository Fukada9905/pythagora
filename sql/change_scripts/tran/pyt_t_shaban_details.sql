DROP TABLE IF EXISTS pyt_t_shaban_details;
delimiter //
/** テーブル作成 **/
CREATE TABLE pyt_t_shaban_details(
...
,	CONSTRAINT FK_pyt_t_shaban_details_pyt_t_shaban
		FOREIGN KEY (id) REFERENCES pyt_t_shaban (id)
		ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='pythagora用 車番明細テーブル';//


/** INSERT(BEFORE) トリガー **/
CREATE TRIGGER pyt_t_shaban_details_BEFORE_INSERT BEFORE INSERT ON pyt_t_shaban_details FOR EACH ROW
BEGIN
...
/** INSERT トリガー **/
CREATE TRIGGER pyt_t_shaban_details_AFTER_INSERT AFTER INSERT ON pyt_t_shaban_details FOR EACH ROW
BEGIN
	INSERT INTO pyt_t_shaban_details_history(
...
/** UPDATE(BEFORE) トリガー **/
CREATE TRIGGER pyt_t_shaban_details_BEFORE_UPDATE BEFORE UPDATE ON pyt_t_shaban_details FOR EACH ROW
BEGIN
...
/** UPDATE トリガー **/
CREATE TRIGGER pyt_t_shaban_details_AFTER_UPDATE AFTER UPDATE ON pyt_t_shaban_details FOR EACH ROW
BEGIN
	INSERT INTO pyt_t_shaban_details_history(
			history_record_divide
		,	id
...
/** DELETE トリガー **/
CREATE TRIGGER pyt_t_shaban_details_AFTER_DELETE AFTER DELETE ON pyt_t_shaban_details FOR EACH ROW
BEGIN
	INSERT INTO pyt_t_shaban_details_history(
			history_record_divide
		,	id
