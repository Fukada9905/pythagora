...
class pdf_101020_index extends pdf_common{
	...
	protected function SetParams(){
		...
		$arr_ptncd = array();
		if($_SESSION["101020_PTNCD"]){
			$arr_ptncd = array_map(function($value){
				return "'".$value["id"] ."'";
			},$_SESSION["101020_PTNCD"]);
		}
		$ptncd = !($arr_ptncd) ? null : implode(",",$arr_ptncd);
		
		$this->params["PTNCD"] = $ptncd;


        $arr_nnsicd = array();
        if($_SESSION["101020_NNSICD"]){
            $arr_nnsicd = array_map(function($value){
                return "'".$value["id"] ."'";
            },$_SESSION["101020_NNSICD"]);
        }
	...
	private function lendarDivide1(){
		...
		foreach ($this->data as $row){
			...
			if($row["DATA_DIVIDE"] === "2" || $row["DATA_DIVIDE"] === "3" || $row["DATA_DIVIDE"] === "4"){
				$cell_params = array(
						"x" => $this->def_x
				,   "y" => $this->def_y + ($this->height * ($this->y_cnt))
				,   "w" => $this->paper_size_w - ($this->def_x * 2)
	...
	private function lendarDivide3(){
		...
		$this->DrawHeaderDivide3($this->data[0],true);
	...
	private function DrawHeaderDivide3($data,$is_first = false){
